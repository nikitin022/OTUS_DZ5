...
## 10. Модель доступа (Row Level Security)

Модель доступа реализована политиками RLS (миграции `0002_rls_policies.sql`, `0003_rls_hardening.sql`):

| Роль | Доступ |
|---|---|
| Гость (`anon`) | Чтение центров, заявок (ленты) и откликов |
| Донор (`authenticated`) | Свой профиль, свои записи, отклики, уведомления, настройки |
| Координатор (`donors.role = 'coordinator'`) | Управление центрами и заявками |
| Системный контур (`service_role`) | Уведомления, история донаций (RLS не применяется) |

Ключевые решения:

- RLS включён на всех 8 таблицах; доступ к приватным данным (записи, уведомления, история) — только владельцу через `auth.uid()`;
- изменение колонки `donors.role` исключено из привилегий роли `authenticated` (`REVOKE UPDATE` + точечный `GRANT`) — клиентский API не может самоприсвоить роль координатора;
- helper-функции `private.auth_donor_id()` и `private.is_coordinator()` (`SECURITY DEFINER`, зафиксированный `search_path`) размещены в схеме `private`, недоступной через REST API;
- политики `auth.uid()` обёрнуты в `(select ...)` — вычисление один раз на запрос (производительность RLS);
- запись уведомлений и истории донаций выполняет только системный контур (`service_role`) — клиентских INSERT-политик нет;
- база развёрнута в управляемом проекте Supabase (регион eu-central-1), миграции применены в порядке `0001 → 0002 → 0003`.